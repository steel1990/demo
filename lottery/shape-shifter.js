...
var S = {
  init: function () {
    ...
    if (i !== -1) {
      ...
      S.UI.simulate('#rectangle 100x100');
    }
    ...
    //   // S.Shape.reset();
    //   S.Shape.switchShape(S.ShapeBuilder.letter('' + x++));
    //   // S.UI.simulate('' + x++);
    // }, 300);
...
S.lottery = function () {
    ...
    function initWithLocalStorage () {
        ...
        nameList = data.nameList || ['屈原', '项羽', '苏武', '嵇康', '陶渊明', '曹植', '曹丕', '曹操', '阮籍', '张融', '王僧孺', '白居易', '杜甫', '李白', '刘禹锡', '元稹', '李商隐', '杜牧', '刘长卿', '温庭筠', '陆游', '杨万里', '苏轼', '黄庭坚', '苏辙', '范成大', '王安石', '欧阳修', '元好问'];
        prizedNameList = data.prizedNameList || [];
    }
    ...
    function disableLottery () {
        isCanLottery = false;
        lotteryBtn.className = 'disable-lottery';
    }
    ...
    function showLotteryResult (name) {
        disableLottery();
        S.UI.reset(true);
        S.UI.simulate('#countdown 3|#rectangle 100x100|' + name);
        setTimeout(enableLottery, 8000);
    }
    ...
    function lottery () {
        var name = nameList[randomInt(0, nameList.length)];
        var i = prizedNameList.length - 1;
        while(i >= 0) {
            if (name === prizedNameList[i]) {
                return lottery();
            }
            i -= 1;
        }

        prizedNameList.push(name);

        saveToLocalStorage(nameList, prizedNameList);
